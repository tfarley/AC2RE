...
public class AttackHook : IHeapObject {
    ...
    public AttackHook(AC2Reader data) {
        ...
        weaponFxCrit = data.ReadEnum<FxId>();
        attackerFxCrit = data.ReadEnum<FxId>();
        missileEntityDid = data.ReadDataId();
        weaponBehaviorHit = data.ReadEnum<BehaviorId>();
        targetBehaviorCrit = data.ReadEnum<BehaviorId>();
        weaponFxMiss = data.ReadEnum<FxId>();
        ...
        targetBehaviorMiss = data.ReadEnum<BehaviorId>();
        targetFxHit = data.ReadEnum<FxId>();
        data.ReadHO<VectorHeapObject>(v => missileSourceOffsetVector = v);
        missileHoldingLocation = data.ReadUInt32();
        attackerFxHit = data.ReadEnum<FxId>();
        flags = data.ReadEnum<Flag>();
        targetFxMiss = data.ReadEnum<FxId>();
        ...
        weaponBehaviorCrit = data.ReadEnum<BehaviorId>();
        weaponBehaviorMiss = data.ReadEnum<BehaviorId>();
        weaponFxHit = data.ReadEnum<FxId>();
        attackerFxMiss = data.ReadEnum<FxId>();
        targetBehaviorHit = data.ReadEnum<BehaviorId>();
        targetFxCrit = data.ReadEnum<FxId>();
    }
}
